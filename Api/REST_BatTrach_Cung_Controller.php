...
class REST_BatTrach_Cung_Controller {
            ...
            array(
                ...
                'args' => [
                    ...
                    'cung' => [
                        'required' => true,
                        'validate_callback' => function($param, $request, $key) {
                            return array_key_exists($param, $this->cungArr);
                        },
                    ],
                ],
...
//            ),
    ...
    public function lay_la_so($data) {
        ...
        if ($tuTru->menhQuaiArr['menhquai']['name2'] != $cung) {
            echo $this->json_response(400, [
                'code' => 'rest_missing_callback_param',
                'message' => '<h3>Bạn không thuộc cung ' . (isset($this->cungArr[$cung]) ? $this->cungArr[$cung]['name'] : '') . '</h3>',
                "data" => [
                    "status" => 400,
                    "params" => [
                        "cung"
        ...
        return new \WP_REST_Response([
            ...
            'bat_trach' => [
                ...
                'bat_trach_theo_24_son_huong' => $this->batTrachTheo24SonHuong($tuTru, $cung),
    ...
    public function batTrachTheo24SonHuong($tuTru, $cung) {
        ob_start();
        ?>
        <h3 class="luan-giai-title">1. LẬP BÁT TRẠCH THEO MỆNH QUÁI 24 SƠN HƯỚNG</h3>
        <?php
        $arrayCung = search_revisions($this->cungArr, $tuTru->menhQuaiArr['menh2'], 'menh');
        moveElement($arrayCung, array_search($cung, $arrayCung), 0);
        get_template_part('xay-nha-template/layout', $tuTru->menhQuaiArr['menh2'], ['arrayCung' => $arrayCung]);
        ?>
        <?php
        return ob_get_clean();
    }
